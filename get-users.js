...
async function getUsers(){
    ...
    db.task( async t => {
        for(let i = 0; i<data.length; i++){
            await t.none(`
                insert into br_bd_users(br_id,name,emp_role_id,designation_name,role,role_name,email,mobile,reportee_br_id,reportee_name,reportee_emp_role_id,created_date,deleted_date,is_available,last_login_date)
                values($(id),$(name),$(emp_role_id),$(designation_name),$(role),$(role_name),$(email),$(mobile),$(reportee_br_id),$(reportee_name),$(reportee_emp_role_id),$(created_date),$(deleted_date),$(is_available),$(last_login_date))
